...
action :install do
  ...
  if install_version || @current_resource.version == nil
	...
	info_output << "(#{can_haz(@new_resource, "preferred_state")})"
...
def upgrade_package(name, version)
  command = "echo -e \"\\r\" | #{@bin} -d preferred_state=#{can_haz(@new_resource, "preferred_state")} upgrade -a#{expand_options(@new_resource.options)} #{prefix_channel(can_haz(@new_resource, "channel"))}#{name}"
  command << "-#{version}" if version and !version.empty?
  pear_shell_out(command)
  manage_pecl_ini(name, :create, can_haz(@new_resource, "directives"), can_haz(@new_resource, "zend_extensions")) if pecl?
end
